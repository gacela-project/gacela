...
- **Event listeners registered in a bootstrap closure and in `gacela.php` all fire.** `Gacela::bootstrap()` builds the setup from the closure and merges `gacela.php` in later, and two things on that path lost listeners in opposite arrangements: the merge *replaced* the generic listeners instead of appending them (specific ones were appended in the very next lines), and the bootstrap dispatches an event before `Config::init()`, memoizing the pre-merge dispatcher so a merge installing a different one left every later dispatch going to the old object. A closure registering generic listeners had its own wiped; one registering none left `gacela.php`'s never firing. Nothing failed either way — the listener was registered and never ran
- **A `#[Cacheable]` custom key is scoped to the class and method that produced it.** The template used to be the whole stored key, so two classes writing the same template — `key: 'user:{0}'`, the one the attribute's own docblock advertises — shared a single entry, and the second to ask was served the first one's data. Nothing failed; the wrong object simply came back. The same gap put those entries out of reach of `clearMethodCacheFor()`, which deletes by the `Class::method::` prefix, so the call that exists to invalidate a method did nothing for exactly the methods that had customised their key — a limitation the docs described rather than a bug they caught. **Stored keys change shape**, so a persistent backend takes one cold pass after upgrading, and a template can no longer share an entry across classes deliberately
- `@method` service resolution reads every form of `use` statement. The parser matched `use A\B;` and `use A\B as C;` with a pattern, so a PSR-12 group import (`use A\B\{C, D};`), a comma list, and a group wrapped across lines by a formatter all matched nothing — and matching nothing did not fail, it fell back to the caller's own namespace. Usually that surfaced as a `MissingClassDefinitionException` for a class the file plainly imports; where a class of the same short name existed in the caller's namespace it was silently injected instead. Now tokenised, which is what this repository does when a wrong match becomes an object rather than a wasted lookup
- A persisted class-name cache entry is checked before it is trusted, and a stale one is re-found and overwritten. Rename or delete a Factory, Config or Provider and deploy without clearing the cache dir, and the entry still resolved — to a class that is gone. Nothing downstream caught it: the container answers `null` for a class it cannot find and `createInstance()` is typed to return an object, so the run died on a `TypeError` naming neither the class, the cache, nor `cache:clear`. It now heals itself with no user action, and the check is free on the warm path because `ClassValidator` memoizes `class_exists` and a surviving name is about to be autoloaded anyway
