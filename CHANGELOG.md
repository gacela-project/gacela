...
- Report a handler registered under a key that cannot satisfy its registry, with `doctor`'s new **handler registries** check. The sibling of the plugin stacks check, for the keyed half of the same idea, and the case is stronger: a registry is looked up *by key*, so the keys a run does not take are exactly the registrations nobody has checked. `LazyHandlerRegistry` raises a `TypeError` for a class that does not exist — on the consumer, wherever that key is first asked for — and a handler that does not implement the contract is not refused at all, so it is handed back and the failure surfaces as a call to a method it does not have. Every finding names the key, since the same handler can be registered under several
...
- Print only the checks that found something with `doctor --only-problems`. Nineteen checks is a lot of "✓" to read to find the one "⚠", and `-q` is not the answer: it suppresses everything, so `--strict -q` fails a build without saying what failed
...
- Report two more ways a `#[Provides]` declaration cannot supply what it promises, and rename the check to **unusable `#[Provides]`** to say so. A method with a **required parameter** is registered and then called with none, so resolving the id raises `ArgumentCountError` at whatever point something first asks for it — loud, but late, and on the consumer rather than the declaration. A method returning **`void`** or **`never`** registers an id that answers `null`, which is as silent as the private-method case the check already reported. An *optional* parameter is not a fault: the scanner calls with none and the default applies
