...
- **The container decorator retained every closure it was ever handed.** `Container` wraps each user closure so service closures receive the decorator rather than the inner container, and marks the wrapper so `set()` does not wrap it twice and silently drop its factory/protected flag. That mark lived in an `SplObjectStorage`, which holds its keys **strongly**, and nothing ever removed an entry — there is no hook for a binding being overwritten or removed. So a container held on to every closure ever passed to `set()`, `bind()`, `extend()`, `factory()` or `protect()` for its whole lifetime, together with everything each one closed over. Overwriting one id 5000 times retained 5000 wrappers and **6.1 MB** to serve a single live binding; `set()` followed by `remove()` retained all of them with **zero** bindings left. Now a `WeakMap`, so a mark lasts exactly as long as the closure it marks is reachable — the same measurements come out at **1.8 KB** and **0.7 KB**. Bounded in a normal bootstrap, where registrations are finite; it bit anything re-registering on a long-lived container, which `createScope()` makes a documented pattern. The `factory()`/`protect()` paths stay retained by the container package's own `FactoryManager`, which has the identical defect — filed as [container#167](https://github.com/gacela-project/container/issues/167), with a test here pinning the residual so it fails when that lands
