...
- Container `^1.3.0` (was `^1.2.1`). 1.3 adds `createScope()` — the parent/child primitive [container#106](https://github.com/gacela-project/container/issues/106) that #539 is blocked on — and `provides()`, and fixes a resolver bug where the bindings map was snapshotted by value, so a `bind()` after the first resolve was invisible to nested constructor resolution. Gacela does not expose scopes yet; that is #539, in 2.1
