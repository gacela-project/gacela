...
A foundation major. The runtime change is two lines — PHP `>=8.3` and `gacela-project/container` `^1.3.0`, up from a `0.x` — and most of what follows comes from the second: `#[Lazy]`, `#[Inject]` on properties, PSR-11-correct `has()`, and container exceptions where 0.x emitted raw PHP errors. Alongside it, the PHPStan suppression Gacela used to ship for undeclared pillar accessors is gone and a Psalm plugin joins the PHPStan extension, so an accessor you have not declared is now reported rather than silently typed `mixed`.
...
- `symfony-bridge/composer.json` now matches the root package it ships from — `php >=8.3` (was `^8.1`), `symfony/dependency-injection` `^7.0 || ^8.0` (was `^6.4 || ^7.0`) and container `^1.3.0` (was `*`). It is autoloaded by the root `composer.json`, so its own constraints were never exercised locally and had silently drifted a major behind
- Container `^1.3.0` (was `^1.2.1`). 1.3 adds `createScope()` — the parent/child primitive [container#106](https://github.com/gacela-project/container/issues/106) that #539 was blocked on — and `provides()`, and fixes a resolver bug where the bindings map was snapshotted by value, so a `bind()` after the first resolve was invisible to nested constructor resolution. Gacela does not expose scopes yet — that is #539, unblocked by this bump and scheduled for 2.1
