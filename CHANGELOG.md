...
- Report a handler that cannot satisfy its registry, with `doctor`'s new **handler registries** check: a class that does not exist, one that does not implement the contract, or a contract that does not exist. `LazyHandlerRegistry` resolves a key on first lookup and never refuses a wrong-typed handler at all, so the keys a run does not take are the registrations nobody has checked. Every finding names the key
...
- Report `debug:dependencies` as a document with `--json`. Its `parameters` and `tree` are the shapes `debug:modules` and `debug:container` already use, so the three commands that describe a class share one vocabulary. `--tree` means what it means in the text report, and every refusal answers with a document too
- Report `debug:container` as a document with `--json`, for both the statistics with the binding map and a class's dependency tree. `processMemoryBytes` is the number rather than `10 MB`, an empty binding map stays `{}`, and `total` counts distinct classes while `tree` repeats a class two parents ask for
- Report `debug:modules` as a document with `--json`: it was the one `--check` command a CI job had to parse prose from. `summary` counts `faults` apart from `unresolvable`, which is the distinction `--check` fails on. `--json` alone is not a gate — only `--check` turns the verdict into an exit code
...
- Report a plugin that cannot satisfy its stack, with `doctor`'s new **plugin stacks** check. `LazyPluginStack` asks the same two questions on first resolve, so a stack nothing has iterated is a registration nobody has checked. A missing class is reported apart from one that does not implement the contract
- Report a pillar file that is on disk and resolved to nothing, with `doctor`'s new **unresolved pillar files** check. A module whose `BlogFactory.php` cannot be loaded is still a module, so `list:modules` printed a blank cell while the file sat there. The filename check does not catch it: the names agree and the namespace is what is wrong
...
- Report a module that was never found, with `doctor`'s new **undiscovered facades** check, naming which fault it is: PHP cannot load the class, or it loads and does not extend `AbstractFacade`. A warning, and only names matching the scaffolder's own pattern count
- Report in `doctor` what a project declared and nothing acts on: an `extendService()` id no Provider `set()`s, a listener target no dispatched event can be, an `addAppConfig()` path matching no file, an unwritable cache directory, a package importing a namespace its `composer.json` never mentions, and stale editor metadata
- Report in `doctor` a `#[Provides]` on a private or protected method — the scanner reads public methods only, so nothing is registered and `getProvidedDependency()` answers `null`
- Report in `doctor` one `#[Provides]` id declared twice on the same Provider: the last method wins and every one before it is dead, with both reading as live
- Report in `doctor` a tagged id nothing can answer, and listeners registered while `disableEventListeners()` is in effect
...
- Report `doctor` as a document with `--format=json`: every check carries its `name`, `status`, `details` and `remediation` under an overall `status`. `--only-problems` narrows it the same way, and the exit code is unchanged
- Report `validate:config` as a document with `--json`, and fail its run on warnings with `--strict`, the bargain `doctor` has always offered
- Emit the effective configuration from `debug:config --json`, for diffing environments. Values keep their own types, and each key carries its `declared`/`undeclared`/`missing` verdict against the schema
- Emit the module inventory from `list:modules --json`; a pillar the module does not have is `null` rather than an omitted key
...
- Point a "cannot resolve" error at the file already in the module directory — one named as expected whose namespace does not match, or one extending the right base under a name no finder rule builds. Read from the file's tokens rather than by name similarity, which would offer `WalletFacade` as the missing Provider
- Name the misnamed file in the "module has no `Factory`" error too, since the common reason a module lands on the stand-in is a misspelled file rather than a missing one
- Refuse an output format the command does not have, in `doctor`, `validate:config`, `debug:config` and `profile:report`, and refuse an unknown `profile:report --sort`. `--format=jsno` printed the text report and exited `0`, so a pipeline could not tell that run from a healthy one. **This changes existing behaviour**: a script passing a format that was silently ignored now fails
...
- Skip loading a file where no class extends anything, since it cannot hold a Facade: 454 of 1627 classes here, discovery from a 151 ms median to 138 ms. Deliberately loose — a false positive costs one load, a false negative silently drops a module
- Ask whether a class is a Facade before building a module out of it: 1292 constructions to keep 121. Discovery from a 251 ms median to 238 ms
...
- Never write or delete cache files at the filesystem root. `''`, `'/'` and whitespace all normalize to an empty directory, so `FileCache` entries landed at `'/<sha1>.php'` and `ScopedCache` persisted its dependency graph to `/`. `CacheFilePath` now answers "nowhere" rather than "the root"
- Hold `APP_ENV` to the same alphabet as a declared config dimension: `APP_ENV=../escaped` wrote the merged-config cache outside its directory, and `APP_ENV=x/../../pwned` failed the write silently and booted uncached
...
- Key the on-disk class-name cache by the bootstrap's `projectNamespaces` and suffix types, so two bootstraps sharing a cache dir no longer serve each other's classes
- Normalize the cache directory on every `getCacheDir()` call, not only the first, so callers no longer disagree about a trailing slash
...
- Refuse `make:module --with-tests` on a template that scaffolds no test, so a reader who asked for a test is not told they have one
- Refuse a `make:module` or `make:file` path whose segments cannot be PHP names, before writing anything. Accented and non-latin names are still accepted
...
- Name the file in "must return a `callable(GacelaConfig)`" and in "The PHP config file must return an array…". Both always said `gacela.php`, so a broken `gacela-prod.php` sent you to the file that was fine
...
- Name the config key that was meant when one is not found: all six typed getters now supply the available keys
...
- Advise on the kind that failed to resolve, name the module-prefixed class in the resolver's `E.g.` line, and carry the class-not-found tips on the two exceptions that name one
- Point the "missing return type" exception at the namespace where `ServiceMap` lives: its recommended fix named one that does not exist
- Say so when a plugin's class does not exist, instead of reporting it as one that "does not implement" the contract
- Allow a Facade to delegate to a kind declared with `addResolvableType()`: `FacadeOnlyDelegates` knew only the three pillar accessors, so two shipped features contradicted each other
- Stop `gacela.suffixExtends` reporting a class that already extends something else, which single inheritance makes impossible
- Stop reporting a Facade's static methods as failing to delegate or as missing from its interface: a static has no `$this` and is not reached through an instance
