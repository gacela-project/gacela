...
- `Gacela::resetCache()` reaches the container's own caches: container 1.4's `Container::resetStaticCaches()` for the four class-shape memos that are `static` and so outlive every container, plus `SharedPlanCache::resetCache()` for the plans. **Not a bug fix** — unlike the `ClassValidator` defect above, which memoized *negatives*, everything here is a positive keyed on a class name, and a class's shape cannot change within a process, so nothing memoized can go stale. What it buys is the invariant `ResetCacheCoverageTest` exists to enforce, and the memory back, in the processes that reset for a living: a worker re-bootstrapping between jobs, code generation, `cache:warm`, a suite declaring fixtures as it goes. Note the guard could not have demanded this — it scans `src/Framework` only, so a vendor class is invisible to it
