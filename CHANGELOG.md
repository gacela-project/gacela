...
- Document `setAppModulePaths()`, which appeared nowhere in the docs despite deciding how much of a project every CLI command walks. With nothing configured the module scan starts at the project root: on this repository that is **14,290 filesystem entries**, against **1,826** when it is narrowed to the directories that actually hold modules — the same modules found either way, and discovery dropping from roughly 220 ms to 157 ms. The difference is `docs/`, `tools/`, `build/` and friends being walked and rejected, and a project with a large `var/` or `storage/` tree saves more. It is a CLI and `cache:warm` cost, not a request-time one, so the note sits with the commands and is linked from the deploy step that pays it

- Preview what a scaffolder would write with `--dry-run`, on `make:module`, `make:file` and `stubs:publish`. `dto:generate` could already answer "what would this do?" and the three commands that write into somebody's project could not — the ones where `--force` replaces hand-written code. The preview resolves its paths through the same code generation uses, so it names the files the real run writes rather than a second guess at them, and it refuses where the real run refuses: a preview over an existing module reports the refusal rather than listing four files nothing would write. "Would replace" is said separately from "would create", because one of them loses whatever is in the file and that is the case somebody runs a preview to check

- Emit the effective configuration from `debug:config --json`. Config drift between environments is a classic production bug, and the only way to compare two of them was to read two tables. The document carries each key with its `declared`/`undeclared`/`missing` verdict against the schema — including the one kind of drift a report built from the values alone cannot show, a declared key no source provides — and, unlike the table, keeps the values as their own types: `true` comes back a boolean and `42` an int, so diffing two environments does not turn every type difference into a text difference. A filter matching nothing is an empty list rather than the sentence the table gives

- Report a module that was never found, with `doctor`'s new **undiscovered facades** check. Every other check works from the modules discovery returned, so one it skipped is invisible to all of them — including the directory scan in the filename check, which walks the directory of each *discovered* module. `list:modules` names the cause only when nothing at all was found, so one broken Facade in fifty left forty-nine modules and silence. A file named the way the scaffolder names a module's Facade — `Blog/BlogFacade.php`, or `Blog/Facade.php` for `--short-name`, under any configured suffix — is now reported when it produced no module, saying which of the two faults it is: PHP cannot load the class, or the class loads and does not extend `AbstractFacade`. Those have different fixes, so the remediation names the right one, and a run with both names both. A warning rather than an error, because `Facade` is an ordinary word and a project beside a framework with its own facades should not have a build failed by a naming coincidence; `--strict` is how a project opts in. Only names matching the scaffolder's own pattern count, so a `NullFacade` or a `FacadeRegistry` is left alone — the first run of this check against gacela's own source reported one of its own DTOs, which is what that rule is for

- Emit the module inventory from `list:modules --json`. Every other way to ask a project what modules it has was built for a reader: this command drew a table, and `debug:module --json` describes the modules matching a *required* argument — so asking it for "everything" meant inventing a substring that happens to match every module, which is not a question it was built to answer and is quietly wrong for a project whose modules share no common fragment. Each entry carries the module name, its full namespace and all four pillars, a pillar the module does not have being `null` rather than an omitted key, so the shape is the same for every module. `--detailed` chooses between two ways of printing and has nothing to say about the document. A filter matching nothing is an empty list rather than the sentence the table gives, because a consumer piping this to a parser got a syntax error exactly when the answer was "none"

- Report `validate:config` as a document with `--json` (or `--format=json`), as `doctor` now does. `--strict` answers "did anything go wrong" with an exit code; which binding, and why, was only ever available as prose. The two commands do not overlap — only `doctor` checks config sources, only this one checks binding cycles — so a job wanting both was running both and could parse just one of them. Each check reports its `name`, `status` and its findings, every finding carrying the same sentence and the same detail lines the text report prints, under an overall `status`; the statuses are the `ok`/`warn`/`error` values `CheckStatus` already carries rather than a second vocabulary. The exit code is unchanged, `--strict` included. This needed the validators to stop writing straight to the output and return what they found, which is why it is not simply a flag: the messages *were* the report. The text output is byte-identical, verified branch by branch against the implementation it replaces
...
- Set a custom event dispatcher from the bootstrap closure with `GacelaConfig::setEventDispatcher()`. `SetupGacela` has always accepted one and `docs/events.md` documents replacing the dispatcher — to bridge onto a PSR-14 bus, or to answer `hasListeners()` yourself so the framework skips allocating the events you do not want — but nothing public could reach it: `Gacela::bootstrap()` hands its closure a `GacelaConfig`, `gacela.php` is handed the same object, and the only route was the `@internal` `Config::createWithSetup()`. A supplied dispatcher takes precedence over `disableEventListeners()`, which governs the dispatcher Gacela would build rather than one it is given

- Report in `doctor` a `#[Provides]` attribute the scanner never reads: the one on a private or protected method. `ProvidesScanner::entriesFor()` reads `getMethods(ReflectionMethod::IS_PUBLIC)`, so PHP accepts the declaration, the method reads as a declared service, and nothing is registered — after which `getProvidedDependency()` for that id answers `null`, which is itself silent, so the first sign is a call on null somewhere else entirely. The finding names the id, the method and its visibility

- Report in `doctor` one `#[Provides]` id declared twice on the same Provider. `ProvidesScanner::scan()` walks the methods in order and `set()`s each id, so the last one wins and every method before it is dead — both read as live, the container answers with one of them, and the only symptom is a value that is not the one the method you are looking at returns. Reported per Provider, because that is where it is unambiguous: each module resolves through its own container, so the same id declared in two modules is two modules answering for themselves rather than a collision. The finding names the id, the Provider and every method claiming it
...
- Print only the checks that found something with `doctor --only-problems`. Fifteen checks is a lot of "✓" to read to find the one "⚠", and `-q` is not the answer: it suppresses everything, so `--strict -q` fails a build without saying what failed
...
- Skip loading a file where no class extends anything. Module discovery loads every php file it walks to ask whether the class descends from `AbstractFacade`, and a file whose classes extend nothing cannot hold one — that is 454 of 1627 classes here, each costing the autoloader a compile of a class the finder then rejects. Discovery drops from a 151 ms median to 138 ms (7 runs per side, same 120 modules) on a project that has narrowed its scan with `setAppModulePaths()`, which is the configuration this matters most for. The check is deliberately loose: it matches anywhere in the file rather than at the start of a line, so a comment, an attribute, a modifier or an `extends` wrapped onto its own line cannot hide a declaration. The two directions are not symmetric — a false positive costs one load, a false negative drops a module from `list:modules`, `doctor`, `debug:graph` and `cache:warm` without saying so
...
- Reattach nine docblocks in the shipped code that described nothing. Adding a method directly above an existing one, between its docblock and its signature, leaves the first undocumented and the second carrying a description of something else — along with whatever `@param` or `@var` the displaced block held. `ConfigResolver::resolve()` and `FactoryResolver::resolve()` had lost their `@param object|class-string $caller` to a property's `@var`; `GacelaConfig::validateConfigSchemaOnBoot()` and `disableEventListeners()`, `FileCache::normalizeDirectory()`, `MergedConfigCache::dimensionSuffix()`, `FileContentGenerator::generate()` and `ConsoleFacade::generateServiceFileContent()` had lost their descriptions, and `FilenameSanitizer::EXPECTED_FILENAMES` had its written below it. Both halves stay valid php, so nothing objected — the analysers only notice when the annotation was load-bearing for a type. A test now walks the shipped directories and fails on any docblock whose next meaningful token is another docblock

- Say whose bindings `debug:module` is listing. The section was headed `Bindings`, under a heading naming one module, beside four sections that really are about that module — and it reports the *application's* container, the same list whichever module is named. It misled in both directions: an id another module's Provider declares read as this module depending on it, and an id absent from the list read as this module not binding it. It is now `Application bindings (project-wide)`. Bindings are registered on one container for the whole project, so there is no module-scoped answer to give here; `Provides (#[Provides])` above it is the section that answers for the module. The `--json` field keeps its name
