...
- Print only the checks that found something with `doctor --only-problems`. Sixteen checks is a lot of "✓" to read to find the one "⚠", and `-q` is not the answer: it suppresses everything, so `--strict -q` fails a build without saying what failed
