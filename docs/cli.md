...
| `debug:module App/Blog` | One module: resolved classes, the ids its Provider declares with `#[Provides]`, the classes it exports as [public API](module-boundaries.md#what-a-module-exports), container bindings, dependency tree |
...
| `debug:container` | Container contents — user bindings and plugins only. A class argument (or `--tree`) draws its dependency tree; `--json` for either as a document. `--stats` also names each [installed package that contributed configuration](packages.md), the file that ran and what it declared, which is the one source in an application a reader cannot find by searching it |
...
| `debug:events` | Every [event](events.md) the framework can dispatch, which of them anything listens to, and which are dispatched on the class-resolution hot path. A listener matches by inheritance, so an event can be covered by a registration that never names it — the listener column names the target that does. An optional argument narrows to class names containing it; `--listened` to only the events something watches; `--json` for a script |
...
| Command | What it does |
|---|---|
| `doctor` | Environmental and wiring health checks, including the [declared config schema](config-schema.md) and each package's `composer.json` against what it imports. Takes an optional namespace to restrict module-scoped checks, `--strict` to exit non-zero on warnings too, `--only-problems` to print just the checks that found something, and `--format=json` for a CI job that wants to say *which* check failed rather than only that one did. Every run names the scope it ran under, because every check works from the modules discovery returned and anything narrowing that narrows all of them at once: the paths walked (`Scanned: src`), any configured entry that is not a directory and so scanned nothing (`Not scanned, not a directory:`), and the filter with how many modules it matched — including when it matched none, which otherwise leaves every module-scoped check reporting "no modules discovered" and the run ending in `All checks passed`. `--format=json` carries the same four as `scanned`, `unscanned`, `filter` and `modules` |
...
Nine of the built-in checks report the same kind of fault: **configuration a project declared that nothing acts on**. Each is accepted at bootstrap, costs nothing at runtime, and does exactly nothing — which is why a command has to be the one to say so. Under `--strict` any of them fails the run.

- **event listeners** — a `registerSpecificListener()` target that is no type at all: a typo, or a class that moved. The dispatcher matches by inheritance, so a parent class or an interface is a legitimate target and is left alone — it covers the family below it — and so is a concrete class nothing dispatches, because that listener is waiting, not broken. What is left is a name nothing can ever be, accepted at bootstrap and never run. It also reports listeners registered while `disableEventListeners()` is in effect: no dispatcher is built, so every one of them is inert, and that is the first thing to suspect when a listener appears dead.
...
- **module paths** — an `appModulePaths` entry that is not a directory. Discovery skips it, and every module-scoped check works from the modules discovery returned, so one mistyped entry narrows all of them at once and the run still ends in a screen of ticks. Gacela does `trigger_error` about it while building the iterator, on a stream a command's own output may not share; the report says it where the reader is, and `Scanned:`/`Not scanned` above the checks show the same split.
...
- **unusable `#[Provides]`** — a declaration that cannot supply what it promises, in any of the three ways PHP accepts. The attribute on a **private or protected** method: `ProvidesScanner` reads public methods only, so the method reads as a declared service and nothing is registered. A method with a **required parameter**: it is registered and then called with none, so resolving the id raises `ArgumentCountError` at whatever point something first asks for it. A method returning **`void`** or **`never`**: the id is registered and answers `null`. The first and third are silent — `getProvidedDependency()` answers `null`, so the first sign is a call on null somewhere else entirely — and the second is loud but late, landing on the consumer rather than the declaration.
...
- **discovered packages** — a `dontDiscover()` entry that refuses nothing. Either it names a package that declares no `extra.gacela.config` — a typo, or a package that stopped shipping one — or it is written in `gacela-{APP_ENV}.php`, which is merged *after* the packages, so the config it names ran and the entry only looks like it worked. The same check reports the other half of [package discovery](packages.md): a declared config file that is not there, or one that does not return a `callable(GacelaConfig)`. Those are skipped at boot on purpose — `composer require` must never be able to stop an application from booting — which is exactly why something has to say so afterwards.
...
The **config environment layers** check names the files a base config path matched that the base layer does *not* read. `addAppConfig('config/*.php')` globs literally, so it also matches the environment files Gacela names itself, and the base layer [excludes them by name](getting-started.md#the-base-layer-excludes-the-environment-files). That rule reads filenames and a filename carries no intent, so a `config/app-extra.php` written for some other purpose is excluded too and read only where the chain resolves to `extra`. Every exclusion is therefore reported: the file, the base file it is taken to refine, and the values that put it in play (`read only when APP_ENV=prod and APP_REGION=eu`). It is a **pass**, not a warning — for any project using `APP_ENV` or a dimension this is what correct looks like — so the remediation is there for the one case it cannot distinguish: rename a file that is not an environment layer, or give it its own `addAppConfig()` path.
...
The **unresolved pillar files** check asks the same question one level down. A module whose `BlogFactory.php` did not resolve is still a module: the Facade resolves, discovery keeps it, and the Factory simply comes back `null` — so `list:modules` prints a blank cell and `debug:module` says `(not found)`, telling you that you have no Factory while you are looking at the file you wrote. It reports a pillar file that is on disk and resolved to nothing, under any configured suffix and for `--short-name` modules too. A module that genuinely has no Factory is the ordinary case and is left alone; the check is about the *file*.

Autoloading is one of the two reasons a pillar comes back `null`, and the other is the pillar's own constructor: an unbound interface throws `DependencyNotFoundException` from a class whose `namespace` is right, whose psr-4 prefix is right, and which loads perfectly. So whatever was thrown is named on the line, and the remediation only offers the `namespace` and `composer dump-autoload` advice when the class genuinely is not there.

The **cacheable storage** check reports `#[Cacheable]` methods running on the default backend. That storage lives and dies with the process, so under PHP-FPM a method annotated for an hour's TTL is recomputed every request and the attribute buys nothing — silently, because a cache that misses looks exactly like one that was never asked to hold anything. It is a warning, not an error: the default is right wherever the process outlives the work, such as a CLI batch or a queue worker. Register a cross-request backend with [`CacheableConfig::setStorage()`](cacheable-methods.md#pluggable-storage-backend), or leave it and let `--strict` decide.
The **plugin stacks** check validates what `addPluginStack()` registered. A stack resolves lazily, so a class that does not exist or does not implement its contract is only refused when something first iterates that stack — which may be a path CI never takes. The check asks the same two questions at deploy time, and reports them apart, because a missing class read as one that "does not implement" the contract sends you to inspect an `implements` clause on a file that is not there.

The **handler registries** check asks the same of `addHandlerRegistry()`, and the case is stronger. A registry is looked up *by key*, so the keys a run does not take are exactly the registrations nobody has checked. `LazyHandlerRegistry` raises a `TypeError` for a class that does not exist — on the consumer, wherever that key is first asked for — and a class that does not implement the contract is not refused at all: it is handed back, and the failure surfaces as a call to a method it does not have. Every finding names the key, because the same handler can be registered under several and the key is what a consumer asks for.
