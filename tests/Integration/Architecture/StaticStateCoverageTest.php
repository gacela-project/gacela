...
final class StaticStateCoverageTest extends TestCase
{
    ...
    private const array OUTLIVES_RESET = [
        ...
        'ClassInfo::$callerClassCache' => 'memoization, self-invalidating: caller class name plus resolvable type to a value object holding a cache key. The key depends on the declared kinds, so the entries are dropped when ResolvableTypes::generation() moves rather than on a cache reset',
        'ClassInfo::$generation' => 'not state: the ResolvableTypes generation the memo above was built under. Clearing it would claim the memo matches the current declarations when it does not',
        'GlobalKey::$cache' => 'memoization, self-invalidating: a class name to its global key string, which names the kind its suffix belongs to. Dropped when ResolvableTypes::generation() moves',
        'GlobalKey::$generation' => 'not state: the ResolvableTypes generation the memo above was built under, as with ClassInfo',
        'ResolvableTypes::$generation' => 'not state: a monotonic counter of how often the declared kinds changed, read by the two memos built from them. It must never go backwards -- a reset to zero would make a memo stamped higher look current',
